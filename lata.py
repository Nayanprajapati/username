...
# fb=Label(N,text='FACEBOOK',fg='blue',font=20)
...
#     c=conn.cursor()
#     c.execute('''insert into user values(:firstName, 
#     :lastName,
#     :age,:adress,
#     :city,:Zipcode,
#     :password,:gender)''',
...
#     sub_btn.grid(row=9,column=0,columnspan=8)
...
# firstname= Entry(N, width=30)
# firstname.grid(row=1,column=1,padx=20,pady=5)

# lastname=Entry(N,width=30)
# lastname.grid(row=2,column=1,pady=5)

# age=Entry(N,width=30)
# age.grid(row=3,column=1,pady=5)

# address=Entry(N,width=30)
# address.grid(row=4,column=1,pady=5)

# city=Entry(N,width=30)
# city.grid(row=5,column=1,pady=5)

# zipcode=Entry(N,width=30)
# zipcode.grid(row=6,column=1,pady=5)

# password=Entry(N,width=30)
# password.grid(row=7,column=1,pady=5)

# gender=Entry(N,width=30)
# gender.grid(row=8,column=1,pady=5)

# fnamelabel= Label(N,text="firstname")
# fnamelabel.grid(row=1,column=0)

# lastnamelabel= Label(N,text="lastname")
# lastnamelabel.grid(row=2,column=0)

# agelabel= Label(N,text="age")
# agelabel.grid(row=3,column=0)

# addresslabel= Label(N,text="address")
# addresslabel.grid(row=4,column=0)

# citylabel= Label(N,text="city")
# citylabel.grid(row=5,column=0)

# zipcodelabel= Label(N,text="zipcode")
# zipcodelabel.grid(row=6,column=0)

# passwordlabel= Label(N,text="password")
# passwordlabel.grid(row=7,column=0)

# genderlabel= Label(N,text="gender")
# genderlabel.grid(row=8,column=0)

# deletelabel= Label(N,text="delete")
# deletelabel.grid(row=10,column=0)

# sub_btn=Button(N,text='Submit',command=submit)
# sub_btn.grid(row=9,column=0,columnspan=2)

# delete_entry=Entry(N,width=30)
# delete_entry.grid(row=10,column=1,pady=5)


# sub_btn=Button(N,text='delete',command=delete)
# sub_btn.grid(row=11,column=0,columnspan=2)
# sub_btn=Button(N,text='show',command=show)
# sub_btn.grid(row=12,column=0,columnspan=2)
# sub_btn=Button(N,text='edit',command=edit)
